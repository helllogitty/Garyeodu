...
const Result = () => {
      ...
      if (data) {
        ...
        if (parsedData.imageUri) {
          ...
          if (parsedData.detectedAreas && parsedData.detectedAreas.length > 0) {
            // 저장된 regionsJson 사용
            handleBlurProcessWithJson(
              parsedData.imageUri,
              parsedData.regionsJson
            );
          } else {
  ...
  const handleBlurProcessWithJson = async (
    imageUri: string,
    regionsJson: string
  ) => {
  ...
  return (
    ...
    >
          ...
          imageDimensions.width > 0 ? (
            <Image
              source={{
                uri: showingBefore
                  ? photoData.imageUri
                  : blurredImageUri || photoData.imageUri,
              }}
              style={{
      ...
      <View
        style={{ gap: 12, marginTop: 20, justifyContent: "flex-end", flex: 1 }}
      >
        <StyledBtn label="편집 완료" isActive={true} onPress={() => {}} />
        <StyledBtn
          label="취소하기"
          isActive={true}
          onPress={() => {
