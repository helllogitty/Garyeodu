 ...
 */
async function removePersonalInfoWithGoogleAI(imagePath: string | Buffer, detectionResult: any) {
  try {
    // 이미지 로드
    let imageBuffer;
    ...
    if (!personalInfoData || personalInfoData.상태 === "안전" || personalInfoData.에러) {
      console.log('개인정보가 감지되지 않았거나 에러가 있습니다. 원본 이미지를 반환합니다.');
      return imageBuffer;
    }
    ...
    Object.entries(personalInfoData).forEach(([key, value]) => {
      ...
      if (
        ...
        const v = value as { 위치: { x: number; y: number; width: number; height: number }; 종류: string };
        const area = {
        ...
        console.log(`제거할 영역: ${area.type} at (${area.left}, ${area.top}) ${area.width}x${area.height}`);
      }
    });
    ...
    const inpaintedBuffer = await performGoogleVisionInpainting(imageBuffer, areasToRemove, width, height);
    
    console.log(`Google AI 개인정보 제거 완료: ${areasToRemove.length}개 영역 처리됨`);
...
async function performGoogleVisionInpainting(
  imageBuffer: Buffer,
  areasToRemove: Array<{ left: number; top: number; width: number; height: number; type: string }>,
  width: number,
  height: number
) {
  try {
    // 1. 이미지 주변 영역 분석으로 컨텍스트 파악
    const contextAnalysis = await analyzeImageContext(imageBuffer, areasToRemove);
...
async function analyzeImageContext(imageBuffer: Buffer, areasToRemove: any[]) {
  try {
    ...
    for (const area of areasToRemove) {
      const surroundingContext = await analyzeSurroundingArea(imageBuffer, area, objects);
      regionContexts.push({
        area: area,
...
async function analyzeSurroundingArea(
  ...
  try {
    ...
    // 주변 영역의 주요 색상 분석
    const [propertiesResult] = await visionClient.imageProperties(surroundingBuffer);
    const dominantColor = propertiesResult.imagePropertiesAnnotation?.dominantColors?.colors[0];
    ...
    for (const obj of objects) {
      ...
      if (Math.abs(objLeft - area.left) < 100 && Math.abs(objTop - area.top) < 100) {
...
function estimateTextureType(
  ...
  if (objectNames.some(name => name.includes('wall') || name.includes('building'))) {
    return 'wall';
  } else if (objectNames.some(name => name.includes('paper') || name.includes('document'))) {
    return 'paper';
  } else if (objectNames.some(name => name.includes('sky') || name.includes('cloud'))) {
    return 'sky';
  } else if (objectNames.some(name => name.includes('fabric') || name.includes('clothing'))) {
    return 'fabric';
  } else {
    return 'general';
  }
...
async function restoreRegionWithGoogleAI(
  imageBuffer: Buffer,
  area: { left: number; top: number; width: number; height: number; type?: string },
  contextAnalysis: {
    ...
    dominantColors: Array<{ color: { red: number; green: number; blue: number } }>;
  },
  width: number,
  height: number
) {
  try {
    // 해당 영역의 컨텍스트 정보 가져오기
    const regionContext = contextAnalysis.regionContexts.find(ctx => 
      ctx.area.left === area.left && ctx.area.top === area.top
    );
    
    if (!regionContext) {
      return await generateContextualReplacement(imageBuffer, area, contextAnalysis.dominantColors[0]);
    }
    ...
    switch (regionContext.textureType) {
      ...
      default:
        return await generateContextualReplacement(imageBuffer, area, regionContext.dominantColor);
    }
    
  } catch (error) {
    console.error('영역 복원 에러:', error);
    return await generateSimpleReplacement(area, contextAnalysis.dominantColors[0]);
  }
}
...
async function smartContextualFill(
  imageBuffer: Buffer,
  areasToRemove: Array<{ left: number; top: number; width: number; height: number }>,
  width: number,
  height: number
) {
  try {
...
async function completeWorkflow(imagePath: string) {
  try {
    ...
    const processedBuffer = await removePersonalInfoWithGoogleAI(imagePath, detectionResult);
    
    // 결과 저장
    const outputPath = imagePath.replace(/\.[^/.]+$/, '_google_ai_processed.jpg');
    await saveProcessedImage(processedBuffer, outputPath);
    
    return {
      원본: imagePath,
      처리완료: outputPath,
      감지된항목: Object.keys(detectionResult["이미지 파일 개인정보 문제"]).length,
...
async function example() {
  try {
    ...
    const processedBuffer = await removePersonalInfoWithGoogleAI('./original-image.jpg', detectionResult);
    
    // 처리된 이미지 저장
    await saveProcessedImage(processedBuffer, './google-ai-processed-image.jpg');
